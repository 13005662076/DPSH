    ...
    for epoch in range(150):
        ...
        for im,label,batch_index in train_loader:
            ...
            if torch.cuda.is_available():
                ...
                S=calS(train_label_onehot,train_labels_onehot)
            else:
                train_label_onehot=EncodingOnehot(label,nclasses)
            ...
            for i,ind in enumerate(batch_index):
                U[ind,:]=output.data[i]
                B[ind,:]=torch.sign(output.data[i])
            Bb=torch.sign(output)
            if torch.cuda.is_available():
                x=output.mm(Variable(U.cuda()).t())/2
                loss1=(Variable(S.cuda())*x-torch.log(1+torch.exp(x))).sum()/(num_train*len(label))
                loss2=(Bb-output).pow(2).sum()/(num_train*len(label))
            else:
                x=output.mm(Variable(U).t())/2
                loss1=(Variable(S)*x-torch.log(1+torch.exp(x))).sum()/(num_train*len(label))
                loss2=(Bb-output).pow(2).sum()/(num_train*len(label))
            '''
            loss1,loss2=criterion(output,S,batch_index,num_train,len(label))
            loss=-loss1+50*loss2 #50是一个超参数，需要根据模型的精确度来确定
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
